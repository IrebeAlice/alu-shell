...
def update_crop_guide(conn, crop_name):
    cursor.execute= extsting_crop()

    # Check if the crop exists in the database
    cursor.execute("SELECT * FROM crop_guide WHERE crop_name=?", (crop_name,))
    ...
    if existing_crop:
        ...
        cursor.execute("UPDATE crop_guide SET growing_conditions=?, planting_care=?, pest_management=?, harvest_storage=? WHERE crop_name=?",
